...
- 宿主 App 必须在自己的 Manifest 中声明 BLE 扫描/连接所需权限；插件只通过 `ActivityPluginBinding` 做运行时检查和权限请求。Android 12+ 使用 Nearby devices（`BLUETOOTH_SCAN` / `BLUETOOTH_CONNECT`），且必须给 `BLUETOOTH_SCAN` 加上 `neverForLocation`，否则系统会把扫描当成定位用途，首次 OTA/扫描可能弹出位置权限。定位权限和定位服务仅 Android 11 及以下需要，请用 `maxSdkVersion="30"` 限制，避免 Android 12+ 包继续声明定位。
